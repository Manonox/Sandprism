...
func _import(vm: LuauVM, v: Vector3, protected: bool) -> bool:
	if !LuaUtils.index_at_path(vm, "Vector3.new"):
		return false
...
func _check(vm: LuauVM, index: int) -> void:
	if vm.lua_type(index) != vm.LUA_TTABLE:
		vm.luaL_typerror(index, "Vector3")
	vm.lua_getfield(index, "__size")
	if !vm.lua_isnumber(-1):
		vm.luaL_argcheck(false, index, "malformed Vector3 argument #%s" % index)
	if !is_equal_approx(vm.lua_tonumber(-1), 3.0):
		vm.luaL_argcheck(false, index, "invalid argument #%s (expected Vector3, got %s)" % [index, LuaUtils.godot_typeof(vm, index)])
	vm.lua_pop()
	vm.lua_getfield(index, "__base")
	if !vm.lua_isvector(-1):
		vm.luaL_argcheck(false, index, "invalid argument #%s (expected Vector3, got table)" % index)
	vm.lua_pop()
